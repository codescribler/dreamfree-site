...
export const enqueueGeneration = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.scheduler.runAfter(0, internal.signalInsightsAction.runInsightGeneration, { insightId });
        return insightId;
    },
});
...
export const getReportsForInsight = internalQuery({
    ...
    handler: async (ctx, args) => {
        ...
        if (!insight)
            return null;
        const reports = await Promise.all(insight.reportsAnalysed.map((id) => ctx.db.get(id)));
        return {
