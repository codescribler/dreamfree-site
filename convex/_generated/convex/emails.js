...
export const sendReportFailureNotification = internalAction({
    ...
    handler: async (_ctx, args) => {
        const apiKey = process.env.RESEND_API_KEY;
        if (!apiKey) {
            console.error("RESEND_API_KEY not set — skipping report failure notification");
            return;
        }
        const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://dreamfree.co.uk";
        const leadLink = `${siteUrl}/dashboard/leads/${args.leadId}`;
        const statusLabel = args.status === "fetch_failed"
        ...
        if (!response.ok) {
            ...
            console.error("Resend API error (report failure notification):", errText);
        }
    },
});
