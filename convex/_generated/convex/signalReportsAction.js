...
import { buildSignalPrompt, calculateOverallScore, } from "../lib/signal-prompt";
...
export const runReportGeneration = internalAction({
    ...
    handler: async (ctx, args) => {
        const { system, user } = buildSignalPrompt(args.strippedContent, args.customerDescription);
        const { primary, fallback } = await ctx.runQuery(internal.aiModels.resolveModelsInternal, { useCase: "signal_reports" });
        let result;
        ...
        if (!isApiReport) {
            adminEmailTasks.push(ctx.runAction(api.emails.sendSignalScoreToVisitor, {
                firstName: args.firstName,
                email: args.email,
                url: args.url,
                overallScore,
        ...
        if (!isApiReport) {
            try {
                const enrollmentId = await ctx.runMutation(internal.emailCampaigns.tryEnrolFromReport, { reportId: args.reportId });
                if (enrollmentId) {
                    await ctx.scheduler.runAfter(0, internal.emailCampaignsAction.generateSequence, { enrollmentId });
                    console.log(`Email campaign enrolment scheduled for report ${args.reportId} → enrollment ${enrollmentId}`);
                }
            }
            catch (err) {
                console.error(`Email campaign enrolment failed for report ${args.reportId}:`, err);
            }
        }
        console.log(`runReportGeneration complete: reportId=${args.reportId} score=${overallScore} model=${modelUsed}`);
    },
});
