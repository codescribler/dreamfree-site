...
export const generateSequence = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        if (!data || !data.enrollment) {
            console.error(`generateSequence: enrollment ${args.enrollmentId} not found`);
            return;
        }
        ...
        const priorDrafts = drafts.map((d) => ({
            role: d.role,
            subject: d.subject,
            bodyText: d.bodyText,
        }));
        // Reconstruct the live loop ledger from the enrollment.
        let loopLedger = [...enrollment.loopLedger];
        const firstNameCandidate = lead.firstName?.trim() || lead.name?.split(" ")[0] || "there";
        for (let i = priorDrafts.length; i < ROLES.length; i++) {
            ...
            try {
                ...
                const primaryMsg = primaryErr instanceof OpenRouterError
                    ? primaryErr.message
                    : String(primaryErr);
                console.warn(`generateSequence primary failed for role=${role} enrollment=${args.enrollmentId}: ${primaryMsg}`);
                try {
                    raw = await callOpenRouter({
            ...
            try {
                ...
                const msg = err instanceof GenerationResultError ? err.message : String(err);
                await ctx.runMutation(internal.emailCampaigns.setEnrollmentStatus, {
                    enrollmentId: args.enrollmentId,
                    status: "generation_failed",
            ...
            loopLedger = [
                ...loopLedger.map((entry) => result.loopsClosed.includes(entry.id) && !entry.closedInRole
                    ? { ...entry, closedInRole: role }
                    : entry),
                ...newlyOpened.filter((n) => !loopLedger.some((existing) => existing.id === n.id)),
            ];
            // Insert the draft (with placeholder unsubscribe token — re-signed below
            // once we have the draft id, since the token includes it.)
            const draftId = await ctx.runMutation(internal.emailCampaigns.insertGeneratedDraft, {
                enrollmentId: args.enrollmentId,
                role,
                order: i,
            ...
            await ctx.runMutation(internal.emailCampaigns.setDraftUnsubscribeToken, { draftId, token: realToken });
            ...
            // resume from.
            await ctx.runMutation(internal.emailCampaigns.updateEnrollmentLoopLedger, { enrollmentId: args.enrollmentId, loopLedger });
            priorDrafts.push({
                role,
                subject: result.subject,
                bodyText: result.bodyText,
