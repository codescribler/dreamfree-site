...
export function buildVerifierUserPrompt(args) {
    const draftsBlock = args.drafts
        .slice()
        .sort((a, b) => a.order - b.order)
        .map((d) => {
        const findings = d.reportFindingsUsed.length > 0
            ? d.reportFindingsUsed.join(", ")
            : "(none cited)";
        const opens = d.loopsOpenedHere.length > 0
            ? d.loopsOpenedHere.join(", ")
            : "(none)";
        const closes = d.loopsClosedHere.length > 0
