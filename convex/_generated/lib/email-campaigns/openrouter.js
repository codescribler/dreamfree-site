...
export async function callOpenRouter(opts) {
    ...
    if (!res.ok) {
        ...
        throw new OpenRouterError(`OpenRouter HTTP ${res.status} ${res.statusText} ${text.slice(0, 300)}`, res.status);
    }
    ...
    if (typeof content !== "string" || content.length === 0) {
        throw new OpenRouterError(`Empty response (finish_reason=${data.choices?.[0]?.finish_reason ?? "unknown"})`);
    }
    return content;
}
/**
