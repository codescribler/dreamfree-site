...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    const filtered = sample.filter(
      (l) => !l.sources.includes("api_outbound") || l.firstEngagedAt != null,
    );
...
export const listWithEngagement = query({
  ...
  handler: async (ctx, args) => {
    ...
    const leadIdSet = new Set(leads.map((l) => l._id as string));
    ...
    for (const r of demoRows) {
      if (leadIdSet.has(r.leadId)) demoLeadIds.add(r.leadId);
    }
    const callbackLeadIds = new Set<string>();
    for (const r of callbackRows) {
      if (leadIdSet.has(r.leadId)) callbackLeadIds.add(r.leadId);
    }
    const formCountByLead = new Map<string, number>();
    for (const r of formRows) {
      if (!r.leadId) continue;
      if (!leadIdSet.has(r.leadId)) continue;
    ...
    const eventsPerLead = await Promise.all(
      leads.map(async (lead) => {
        const events = await ctx.db
          ...
          .order("desc")
          .take(20);
        return { leadId: lead._id as string, events };
      }),
    );
    ...
    for (const { events } of eventsPerLead) {
      for (const e of events) {
        ...
        if (typeof props.url === "string" && props.url.length > 0) continue;
        const m = e.path.match(reportPathRe);
        if (m) reportIdSet.add(m[1]);
      }
    }
    const reportUrlById: Record<string, string> = {};
    await Promise.all(
      Array.from(reportIdSet).map(async (id) => {
        try {
          const r = await ctx.db.get(id as Id<"signalReports">);
          if (r) reportUrlById[id] = r.url;
        } catch {
          // Bad id — skip.
        }
      }),
    ...
    function enrich(e: Doc<"events">): Doc<"events"> {
      const props = (e.properties ?? {}) as Record<string, unknown>;
      if (typeof props.url === "string" && props.url.length > 0) return e;
      const m = e.path.match(reportPathRe);
      if (!m) return e;
    ...
    const latestEventByLeadId: Record<string, Doc<"events"> | null> = {};
    const engagementByLeadId: Record<
      string,
      {
        eventCount: number;
        viewedReportCount: number;
    ...
    for (const { leadId, events } of eventsPerLead) {
      ...
      const viewedReportCount = events.filter(
...
export const markEngaged = mutation({
  args: { leadId: v.id("leads") },
  handler: async (ctx, args): Promise<{ ok: boolean; alreadyEngaged: boolean }> => {
    const lead = await ctx.db.get(args.leadId);
    if (!lead) return { ok: false, alreadyEngaged: false };
