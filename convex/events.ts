...
export const recentActivity = query({
  ...
  handler: async (ctx, args) => {
    ...
    const reportIds = new Set<Id<"signalReports">>();
    for (const e of rawEvents) {
      const props = (e.properties ?? {}) as Record<string, unknown>;
      const propUrl = typeof props.url === "string" ? props.url : null;
      if (propUrl) continue; // already enriched (outbound_report_viewed)
      const match = e.path.match(reportPathRe);
      if (match) reportIds.add(match[1] as Id<"signalReports">);
    }
    ...
    await Promise.all(
      Array.from(reportIds).map(async (id) => {
        try {
          ...
          if (report) reportUrlById[id] = report.url;
        } catch {
          // Bad id, skip.
        }
      }),
